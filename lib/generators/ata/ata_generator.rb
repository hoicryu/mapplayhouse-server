class AtaGenerator < Rails::Generators::NamedBase
  ...
  def image_url?
    model_constant.included_modules.include?(ImageUrl)
  rescue StandardError
    false
  end

  def imagable?
    model_constant.included_modules.include?(Imagable)
  rescue StandardError
    false
  end

  def likable?
    model_constant.included_modules.include?(Likable)
  rescue StandardError
    false
  end

  def commentable?
    model_constant.included_modules.include?(Commentable)
  rescue StandardError
    false
  end
end
